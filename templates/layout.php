...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  foreach ($required as $key) {
    ...
    if(count($errors) == 0) {
        array_unshift ( $projects_table, [
            'task' => $_POST['name'],
            'date' => '01.05.2018',
            'categories' => $_POST['project'],
            'status' => false
        ] );
    }
...
?>
<?php if (isset($_GET['add']) || ($form_sent == true && count($errors) > 0)): ?>
    <body class="overlay">
      <?php
       $modal_task = get_template('templates/modal-task.php', [
 'errors' => $errors
]);
